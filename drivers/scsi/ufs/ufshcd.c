...
static void ufshcd_pm_qos_get_worker(struct work_struct *work)
{
	...
	if (atomic_read(&hba->pm_qos.count) && !hba->pm_qos.active) {
		pm_qos_update_request(&hba->pm_qos.req, 44);
...
#if defined(CONFIG_UFSFEATURE) && defined(CONFIG_UFSFEATURE30)
...
#if defined(CONFIG_UFSFEATURE) && defined(CONFIG_UFSFEATURE31)
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	/* IRQ registration */
	err = devm_request_irq(dev, irq, ufshcd_intr, IRQF_SHARED | IRQF_PRIME_AFFINE,
				dev_name(dev), hba);
	if (err) {
